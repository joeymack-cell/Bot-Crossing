...
export class Navigation {
  ...
  /** Disconnected courtyards are free ground, but the landing crew cannot reach them. */
  _labelRegions() {
    const { regions, regionQueue: queue, blocked, size } = this
    regions.fill(0)
    let region = 0
    for (let i = 0; i < blocked.length; i++) {
      if (blocked[i] || regions[i]) continue
      let head = 0, tail = 1
      queue[0] = i; regions[i] = ++region
      while (head < tail) {
        const at = queue[head++], x = at % size, z = Math.floor(at / size)
        // Four-way connectivity matches A*'s no-corner-cutting rule.
        for (let k = 0; k < 4; k++) {
          const nx = x + NEIGHBOURS[k * 2], nz = z + NEIGHBOURS[k * 2 + 1]
          if (!this.inBounds(nx, nz)) continue
          const next = nz * size + nx
          if (blocked[next] || regions[next]) continue
  ...
  /** Closest stand position in the same walkable region as the arrival point. */
  reachablePoint(sx, sz, tx, tz) {
    const start = this.nearestFree(sx, sz)
    if (!start) return null
    const region = this.regions[start.iz * this.size + start.ix]
    const ix = this.toCell(tx), iz = this.toCell(tz)
    if (this.inBounds(ix, iz) && !this.isBlocked(tx, tz) && this.regions[iz * this.size + ix] === region) return { x: tx, z: tz }
    let best = -1, distance = Infinity
    for (let i = 0; i < this.regions.length; i++) {
      if (this.regions[i] !== region) continue
      const x = this.toWorld(i % this.size), z = this.toWorld(Math.floor(i / this.size))
      const d = (x - tx) ** 2 + (z - tz) ** 2
      if (d < distance) { distance = d; best = i }
    }
    return best < 0 ? null : { x: this.toWorld(best % this.size), z: this.toWorld(Math.floor(best / this.size)) }
  }
  ...
  findPath(sx, sz, tx, tz) {
    ...
    if (this.regions[startIdx] !== this.regions[goalIdx]) return null
  ...
  slide(pos, dx, dz) {
    ...
    const nx = pos.x + dx
    const nz = pos.z + dz
    const cx = this.toCell(pos.x), cz = this.toCell(pos.z)
    const tx = this.toCell(nx), tz = this.toCell(nz)
    const cutsCorner = cx !== tx && cz !== tz && (this.isBlocked(nx, pos.z) || this.isBlocked(pos.x, nz))
    if (!cutsCorner && this.lineOfSight(pos.x, pos.z, nx, nz)) {
      pos.x = nx
      pos.z = nz
      return true
    }
    if (Math.abs(dx) > 1e-6 && this.lineOfSight(pos.x, pos.z, nx, pos.z)) {
      pos.x = nx
      return true
    }
    if (Math.abs(dz) > 1e-6 && this.lineOfSight(pos.x, pos.z, pos.x, nz)) {
      pos.z = nz
      return true
    }
    return false
