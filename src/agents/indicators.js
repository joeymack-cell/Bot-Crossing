...
export class Indicators {
  ...
  update(agents, elapsed, statusFor) {
    ...
    const reduced = this.settings.get('reducedMotion') || window.matchMedia('(prefers-reduced-motion: reduce)').matches
    let n = 0
    ...
    for (const agent of agents) {
      ...
      const bob = reduced ? 0 : Math.sin(elapsed * bobRate + agent.phase) * bobAmp
      ...
      centers[n * 3 + 1] = agent.pos.y + HEAD_CLEAR * (agent.isMini ? 0.58 : 1) + bob
