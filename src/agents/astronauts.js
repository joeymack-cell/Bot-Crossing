...
import { recencyFor } from './recency.js'
import { personalityFor } from './personality.js'
import { sectionColor } from '../game/sections.js'
...
export class Astronauts {
  ...
  _buildMeshes(capacity) {
    ...
    this.backpacks = new Backpacks(this.group, capacity)
    this.backpacks.setShadows(this.settings.get('shadows') !== 'off')
  ...
  onSettingsChanged(changed) {
    if (changed.has('shadows')) { this._applyShadowFlags(); this.backpacks.setShadows(this.settings.get('shadows') !== 'off') }
  ...
  setRoster(entries, world) {
    const initialRoster = !this.roster?.length
    this.roster = entries
    this.world = world || this.world
    const cap = Math.min(this.capacity, this.settings.get('maxAgents'))
    const wanted = visibleRoster(entries, cap)
    const seen = new Set()
    ...
    for (const entry of wanted) {
      ...
      else this._spawnAgent(entry, !entry.thread.isMini && (initialRoster || entry.thread.createdAt < Date.now() - 60000))
    }

    for (const agent of this.agents) {
      // Date filtering and the crew budget hide models immediately.
      if (!seen.has(agent.id)) {
        if (agent.isMini && agent.state !== 'gone') {
          if (agent.state !== 'leaving') {
            this._sendHome(agent)
            const parent = this.byId.get(agent.thread.parentId) || this.byId.get(agent.thread.rootParentId)
            if (parent) agent.site.copy(parent.pos)
          }
        } else agent.state = 'gone'
      }
    }
    return this.agents.length
  }

  _spawnAgent(entry, alreadyHere = false) {
    const parent = entry.thread.isMini && (this.byId.get(entry.thread.parentId) || this.byId.get(entry.thread.rootParentId))
    const door = (alreadyHere && entry.site) || parent?.pos || this.world?.shipDoor?.() || new THREE.Vector3(0, 0, 0)
    const jitter = () => alreadyHere ? 0 : (Math.random() - 0.5) * 1.4

    const agent = {
      id: entry.id,
      personality: personalityFor(entry.id),
      sectionColor: entry.sectionColor || parent?.sectionColor || sectionColor(entry.id),
      isMini: !!entry.thread.isMini,
      attention: null,
      thread: entry.thread,
      status: entry.status,
      ...
      targetYaw: 0,
      speed: WALK_SPEED * (entry.thread.isMini ? 2 : 1) * (0.86 + Math.random() * 0.28),
      ...
      faceTimer: 0,
      faceIndex: 0,
      suit: recencyFor(entry.thread).color,
    ...
    // Spawn jitter must stay on the ramp's reachable ground, never in a sealed gap.
    const spawn = this.nav?.reachablePoint(door.x, door.z, agent.pos.x, agent.pos.z)
    if (spawn) agent.pos.set(spawn.x, 0, spawn.z)
    this._applyStatus(agent, entry.status)
    // Existing sessions are already at work when the city opens. Only newcomers use the ramp.
    if (alreadyHere) { agent.state = 'at-site'; agent.scale = 1 }
  ...
  _updateAgent(agent, entry) {
    if (agent.state === 'gone' || agent.state === 'leaving') {
      agent.state = 'walking'; agent.stateAge = 0; agent.pathVersion = -1
    }
    agent.thread = entry.thread
    if (entry.sectionColor && entry.sectionColor !== agent.sectionColor) {
      agent.sectionColor = entry.sectionColor
      agent.eye.set(agent.sectionColor).multiplyScalar(1.6)
      agent.colorDirty = true
    }
    const suit = recencyFor(entry.thread).color
    if (suit !== agent.suit) {
      agent.suit = suit
      agent.colorDirty = true
    }
  ...
  /** Status changes behaviour and trim; the face keeps its section color. */
  ...
  update(dt, elapsed, camera) {
    const reduced = this.settings.get('reducedMotion') || this.motionPreference.matches
    ...
    for (let i = this.agents.length - 1; i >= 0; i--) {
      ...
      agent.stateAge += dt
      if (!this._greetStep(agent, dt, camera, reduced)) this._step(agent, dt, elapsed, anim)
  ...
  /** Immediate face feedback; a nearby camera earns one interruptible greeting. */
  greet(id) {
    const agent = this.byId.get(id)
    if (!agent || agent.state === 'gone') return
    if (agent.attention?.active) return // rapid clicks do not restart an arm mid-wave
    agent.attention = { pending: true, active: false, elapsed: 0, notice: 1.5, style: agent.personality.greeting }
    agent.faceFrame = FACE.happy
    agent.blinkAt = 1.5
  }

  _greetStep(agent, dt, camera, reduced) {
    const attention = agent.attention
    if (!attention) return false
    attention.notice = Math.max(0, attention.notice - dt)
    if (this.selected !== agent || !camera) { agent.attention = null; return false }
    const distance = camera.position.distanceTo(agent.pos)
    if (attention.pending && distance < 30 && agent.scale > 0.8) {
      attention.pending = false
      attention.active = true
      attention.elapsed = 0
    }
    if (!attention.active) return false
    attention.elapsed += dt
    if (attention.elapsed > (reduced ? 1.5 : 3.0) || distance > 38) {
      attention.active = false
      agent.pathVersion = -1
      return false
    }
    if (reduced) return false // expression feedback only; no extra body movement
    agent.vel.set(0, 0, 0)
    agent.groundSpeed = 0
    this._faceToward(agent, camera.position, dt)
  ...
  _steerTarget(agent, out) {
    ...
    while (agent.pathAt < path.length - 1) {
      ...
      if (!nav.lineOfSight(agent.pos.x, agent.pos.z, next.x, next.z)) break
      agent.pathAt++
    }
  ...
  _step(agent, dt, elapsed, anim) {
    ...
    switch (agent.state) {
      ...
      case 'walking': {
        ...
        // A crowded entrance is never a replacement for the agent's own building.
        // Retry a slow route; long walks across a large city are allowed to finish.
        if (agent.stateAge > 8 && agent.blocked) {
          agent.pathVersion = -1
          agent.stateAge = 0
        }
        if (dist < ARRIVE_RADIUS && (!this.nav || this.nav.lineOfSight(agent.pos.x, agent.pos.z, agent.site.x, agent.site.z))) {
  ...
  _walk(agent, toTarget, goalDist, dt, factor) {
    ...
    if (this.nav) {
      ...
      // walk through a wall.
      const fromX = agent.pos.x, fromZ = agent.pos.z
      const slid = this.nav.slide(agent.pos, dx, dz)
      if (!slid || (Math.hypot(dx, dz) > .001 && Math.hypot(agent.pos.x - fromX, agent.pos.z - fromZ) < .00001)) {
  ...
  _face(agent, dt) {
    agent.faceTimer += dt
    agent.blinkAt -= dt
    if (agent.attention?.active || agent.attention?.notice > 0) {
      agent.faceFrame = agent.attention?.style === 'dance' ? FACE.cheer : FACE.happy
      return
    }
    ...
    if (!loop || !loop.length) {
      agent.faceFrame = FACE[agent.personality.expression] ?? FACE.idle
      return
    }
  ...
  _animate(agent, dt, anim) {
    ...
    let key
    const greeting = agent.attention?.active && !this.settings.get('reducedMotion') && !this.motionPreference.matches
    if (greeting) key = agent.attention.style === 'dance' ? 'cheer' : 'wave'
      ...
      switch (agent.status) {
        ...
        case 'sleeping':
          ...
          break
        default:
          key = agent.personality.idleClip
      }
    }
    ...
    const rate = key === 'walk' || key === 'run' ? THREE.MathUtils.clamp(speed / (WALK_SPEED * (agent.isMini ? 0.58 : 1)), 0.4, agent.isMini ? 3.8 : 2.1) : 1
  ...
  _writeMatrices(elapsed, anim) {
    ...
    for (const agent of this.agents) {
      ...
      const greeting = agent.attention
      const dancing = greeting?.active && greeting.style === 'dance' && !this.settings.get('reducedMotion') && !this.motionPreference.matches
      const beat = dancing ? Math.sin(greeting.elapsed * Math.PI * 4) * Math.sin(Math.min(1, greeting.elapsed / 0.3) * Math.PI / 2) * Math.min(1, (3 - greeting.elapsed) / 0.3) : 0
      e.set(0, agent.yaw + beat * 0.16, beat * 0.045)
      q.setFromEuler(e)
      v.set(agent.pos.x, agent.pos.y, agent.pos.z)
      root.compose(v, q, one.setScalar(s * CREW_SCALE * (agent.isMini ? 0.58 : 1)))
      one.setScalar(1)
      ...
      if (rig) {
        ...
        this.backpacks.write(agent.personality, worn)
      ...
      // Suit and trim change when age/status changes, or when an agent leaving the roster
  ...
  pick(camera, ndcX, ndcY, aspect, maxDist = 0.075) {
    ...
    for (const agent of this.agents) {
      ...
      v.set(agent.pos.x, agent.pos.y + (this.headHeight || 0.75) * (agent.isMini ? 0.58 : 1), agent.pos.z).project(camera)
  ...
  setSelected(agent) {
    if (this.selected && this.selected !== agent) this.selected.attention = null
