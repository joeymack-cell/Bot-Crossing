import { personalityFor } from '../agents/personality.js'
import { PROJECT_BADGES } from '../game/history.js'
...
import { RECENCY_BANDS, recencyFor } from '../agents/recency.js'
...
export class Hud {
  ...
  _buildSettings() {
    ...
    view.append(
      ...
      this._toggle('Project signs', 'showLabels'),
  ...
  _wire() {
    ...
    on('#btn-chat', 'click', () => this.actions.showChat?.())
    ...
    on('#section-color', 'input', e => this.actions.saveSectionColor?.(e.target.value))
    on('#history-days', 'change', e => this.settings.set('historyDays', Number(e.target.value)))
    on('#btn-kept', 'click', () => this.actions.keepThread?.())
    on('#btn-rename', 'click', () => {
      this.$('#rename-form').hidden = false
      this.$('#session-name').value = this.selected?.thread.title || ''
      this.$('#session-name').focus()
      this.$('#session-name').select()
      this.measureCard()
    })
    on('#rename-cancel', 'click', () => this.finishRename())
    on('#rename-form', 'submit', e => { e.preventDefault(); this.actions.renameThread?.(this.$('#session-name').value) })
    on('#project-sign', 'submit', e => {
      e.preventDefault()
      this.actions.saveProject?.(this.$('#project-display-name').value, this.$('#project-badge').value)
    })
    on('#btn-archives', 'click', () => {
      const panel = this.$('.archive-panel')
      panel.hidden = !panel.hidden
      this.$('#btn-archives').setAttribute('aria-expanded', String(!panel.hidden))
    })
    on('#close-archives', 'click', () => { this.$('.archive-panel').hidden = true; this.$('#btn-archives').setAttribute('aria-expanded', 'false') })
    on('#archive-search', 'input', () => this.renderArchives())
  ...
  syncSettings() {
    ...
    this.$('#history-days').value = String(this.settings.get('historyDays'))
  ...
  setLegend(projects, activeName = null) {
    const signature = projects.map((p) => `${p.name}:${p.identity?.displayName}:${p.identity?.badge}:${p.count}:${p.accent}:${p.urgent ? 1 : 0}`).join('|') + `~${activeName}`
    ...
    for (const p of projects) {
      ...
      b.innerHTML =
        ...
        `<span class="project-emoji" aria-hidden="true">${p.identity?.icon || "🚀"}</span><span class="n">${escapeHtml(p.identity?.displayName || p.name)}</span>` +
  ...
  setProject(project) {
    ...
    const signature =
      `${project.identity?.displayName}~${project.identity?.badge}~${project.name}~${project.path}~${project.accent}~${project.selectedId}~${Math.floor(Date.now() / 60000)}~` +
      project.threads.map((t) => `${t.id}:${t.keepVisible}:${t.status}:${t.title}:${t.lastActivityAt}`).join('|')
    ...
    this.$('.side .name').textContent = (project.identity?.icon || '') + ' ' + (project.identity?.displayName || project.name)
    if (!this.$('#project-sign').contains(document.activeElement)) {
      this.$('#project-display-name').value = project.identity?.displayName || project.name
      this.$('#project-badge').value = project.identity?.badge || 'rocket'
    }
    ...
    for (const t of project.threads) {
      ...
      b.innerHTML =
        '<i class="pip"></i>' +
        `<span class="t">${t.keepVisible ? "📌 " : ""}${escapeHtml(t.title || 'Untitled thread')}</span>` +
    ...
    if (!n) {
      ...
      empty.textContent = 'No sessions in this range. Expand History above to see older work.'
  ...
  setSelection(agent, thread) {
    ...
    if (this.selected?.thread.id !== thread.id) this.finishRename()
    ...
    if (thread.isMini) bits.push('<span class="tag">Mini helper · 2× speed</span>')
    const personality = agent.personality || personalityFor(thread.id)
    bits.push('<span class="tag" title="This session keeps its backpack and personality">' + personality.backpack.icon + ' ' + escapeHtml(personality.backpack.label) + ' · ' + personality.name + '</span>')
    const recency = recencyFor(thread)
    bits.push(`<span class="tag" title="Suit color follows last activity"><i class="swatch" style="background:${recency.css}"></i>${recency.label}</span>`)
    ...
    this.$('#btn-open').title = this.el.closest('#app')?.classList.contains('studio-embed') ? 'Open this session in Dev Studio' : 'Open this session'
    this.$('#section-color').value = this.actions.sectionColor?.(thread.id) || '#c96442'
    this.$('.section-color-row').hidden = !!thread.isMini
    this.$('#btn-archive').innerHTML = ICON.archive + ' Archive in Codex'
    this.$('#btn-archive').disabled = thread.canArchive === false || !!thread.running || !!this.taskBusy
    this.$('#btn-archive').title = thread.running ? 'Wait until the task finishes' : 'Archive this session and its spawned tasks in Codex. Restore it from Archived sessions.'
    this.$('#btn-rename').disabled = !thread.canRename || !!this.taskBusy
    this.$('#btn-kept').textContent = thread.keepVisible ? '📌 Kept visible' : '📌 Keep visible'
    this.$('#btn-kept').setAttribute('aria-pressed', String(!!thread.keepVisible))
    this.measureCard()
  }
  ...
  finishRename() {
    this.$('#rename-form').hidden = true
    this.measureCard()
  }

  setTaskBusy(busy) {
    this.taskBusy = busy
    this.$('#rename-save').disabled = busy
    this.$('#rename-cancel').disabled = busy
    this.$('#session-name').disabled = busy
    this.$('#btn-rename').disabled = busy || !this.selected?.thread.canRename
    this.$('#btn-archive').disabled = busy || !!this.selected?.thread.running
    this.$('#btn-archive').textContent = busy ? 'Saving…' : 'Archive in Codex'
    for (const button of this.$('.archive-list').querySelectorAll('button')) button.disabled = busy
  }

  setHistory(view) {
    this.archivedThreads = view.archived
    this.$('#history-note').textContent = view.visible.length + ' shown · ' + view.hiddenCount + ' older hidden'
    this.$('#btn-archives').textContent = 'Archived sessions (' + view.archived.length + ')'
    this.renderArchives()
  }

  renderArchives() {
    const query = this.$('#archive-search').value.toLowerCase().trim()
    const list = (this.archivedThreads || []).filter(t => (t.title + ' ' + t.project).toLowerCase().includes(query))
    const signature = JSON.stringify(list.map(t => [t.id,t.title,t.project]))
    if (signature === this._last.archives) return
    this._last.archives = signature
    const wrap = this.$('.archive-list')
    ...
    for (const thread of list) {
      ...
      const project = document.createElement('small'); project.textContent = thread.project
      const button = document.createElement('button'); button.className = 'btn'; button.type = 'button'; button.textContent = 'Restore'; button.disabled = !!this.taskBusy
      button.addEventListener('click', () => this.actions.restoreThread?.(thread.id))
      copy.append(name,project); row.append(copy,button); wrap.appendChild(row)
    }
    if (!list.length) { const p = document.createElement('p'); p.className = 'empty-note'; p.textContent = query ? 'No archived sessions match.' : 'No archived sessions.'; wrap.appendChild(p) }
  }
  ...
  placeCard(screen) {
    ...
    const top = Math.max(margin, Math.min(Math.max(margin, screen.y - size.h / 2), window.innerHeight - margin - size.h - (this._bottomInset || 0)))
  ...
  updateAvatar(faceAtlasCanvas) {
    ...
    const css = agent.sectionColor || cssFromGlow(color)
...
<aside class="side panel">
  ...
  <div class="stats"></div>
  <button class="btn city-chat-launch" id="btn-chat">Open chat</button>

  <section class="recency-legend" aria-label="Agent suit colors by last activity">
    <div class="recency-heading">Suit color · last active</div>
    <div class="recency-bands">${RECENCY_BANDS.map(b => `<span title="${b.label}"><i style="background:${b.css}" aria-hidden="true"></i>${b.shortLabel}</span>`).join('')}</div>
  </section>

  <section class="history-controls" aria-label="Session history">
    <div class="history-row"><label for="history-days">History</label>
    <select id="history-days" class="select"><option value="1">Last day</option><option value="3">Last 3 days</option><option value="7">Last 7 days</option><option value="14">Last 14 days</option><option value="30">Last 30 days</option><option value="60">Last 60 days</option><option value="90">Last 90 days</option><option value="0">All time</option></select></div>
    <p id="history-note"></p>
    <button class="btn ghost" id="btn-archives" aria-expanded="false">Archived sessions</button>
  </section>
  <div class="side-body">
    ...
    <div class="project-detail">
      <button class="btn ghost back" id="btn-close-project" title="Back to every repo (Esc)">${ICON.back} All projects</button>
      <div class="who">
        <i class="swatch"></i>
        <div class="text">
          <div class="name"></div>
      ...
      <details class="sign-editor"><summary>Headquarters badge &amp; sign</summary>
        <form id="project-sign">
          <label for="project-display-name">Sign name <small>Colony display only</small></label>
          <input id="project-display-name" maxlength="48" required autocomplete="off">
          <label for="project-badge">Project badge</label>
          <select id="project-badge" class="select">${Object.entries(PROJECT_BADGES).map(([id,b]) => '<option value="'+id+'">'+b.icon+' '+b.label+'</option>').join('')}</select>
          <button class="btn" type="submit">Save sign</button>
        </form>
      </details>
...
<div class="thread-pop panel">
  ...
  <div class="pair session-tools"><button class="btn" id="btn-rename">Rename session</button><button class="btn" id="btn-kept" aria-pressed="false">📌 Keep visible</button></div>
  <form id="rename-form" hidden>
    <label for="session-name">Section / session name</label>
    <input id="session-name" maxlength="160" required autocomplete="off">
    <div class="pair"><button class="btn primary" id="rename-save" type="submit">Save name</button><button class="btn" id="rename-cancel" type="button">Cancel</button></div>
  </form>
  <label class="section-color-row" for="section-color"><span>Section color</span><input type="color" id="section-color" title="Choose this section's building color"></label>
  <div class="pair">
...
<section class="archive-panel panel" hidden aria-label="Archived sessions">
  <header><h2>Archived sessions</h2><button class="btn icon ghost" id="close-archives" aria-label="Close archived sessions">${ICON.close}</button></header>
  <p>Restore finished work to Codex whenever you need it.</p>
  <label class="sr-only" for="archive-search">Search archived sessions</label><input id="archive-search" type="search" placeholder="Search sessions or projects…">
...
<div class="help">
  <div class="sheet panel">
    ...
    <div style="margin-top:16px">
      ...
      <div class="legend-row"><i class="badge" style="background:#16301f;color:#7fd39a">⚒</i> working right now</div>
