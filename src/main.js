...
import { historyView, projectIdentity } from './game/history.js'
import { sectionColor, mergeStudioThreads } from './game/sections.js'
import { Hud } from './ui/hud.js'
import { CityChat } from './ui/chat.js'
...
const studioEmbed = window.parent !== window && new URLSearchParams(location.search).get('studio') === '1'
let studioChat = studioEmbed
let studioInsets = { right: 400, bottom: 200 }
app.classList.toggle('studio-chat', studioChat)
app.classList.toggle('studio-embed', studioEmbed)
...
let state = { archived: [], archivedAt: {}, opened: [], plots: {}, seen: {}, kept: [], projects: {} }
let threads = []
let scannerThreads = []
let studioThreads = []
...
const actions = {
  showChat: () => {
    if (!chat) return
    const thread = threads.find(t => t.id === selectedId && t.harness === 'codex') || chat.current || threads.filter(t => t.harness === 'codex' && !t.archived && !t.isMini).sort((a,b) => b.lastActivityAt-a.lastActivityAt)[0]
    if (thread) void chat.open(thread)
    else hud.toast('Choose a project to start a conversation.')
  },
  ...
  focusProject: (name) => {
    ...
    if (!plot) return
    rig.focus(plot.worldSlot(0).setY(5), { distance: 44 })
  },
  ...
  select: (id) => select(id, { react: true }),

  focusThread: (id) => select(id, { fly: true, react: true }),
  ...
  newConversation: async () => {
    if (studioEmbed) { window.parent.postMessage({ type: 'bot-crossing:new-session' }, '*'); return }
    ...
    try {
      ...
      if (chat && harness === 'codex') { await chat.open({ folder, project: name, title: 'New conversation' }); return }
  ...
  openThread: async () => {
    ...
    if (!thread) return
    if (studioEmbed) {
      window.parent.postMessage({ type: 'bot-crossing:open-session', id: thread.studioConversationId ? 'studio:' + thread.studioConversationId : thread.id }, '*')
      return
    }
    try {
      if (chat && thread.harness === 'codex') { await chat.open(thread); return }
  ...
  keepThread: () => {
    const thread = threads.find(t => t.id === selectedId)
    if (!thread) return
    if (thread.starred) return hud.toast('This task is pinned in Codex and always stays visible.')
    const kept = new Set(state.kept || [])
    kept.has(thread.id) ? kept.delete(thread.id) : kept.add(thread.id)
    state.kept = [...kept]
    queueSave()
    applyThreads(threads)
    hud.toast(kept.has(thread.id) ? 'Kept visible across every history range' : 'This task now follows the history filter')
  },
  renameThread: async name => {
    const thread = threads.find(t => t.id === selectedId)
    if (!thread || hud.taskBusy) return
    if (studioEmbed && thread.studioConversationId) {
      window.parent.postMessage({ type: 'bot-crossing:rename-session', id: thread.studioConversationId, title: name.trim().slice(0,160) }, '*')
      hud.finishRename()
      return
    }
    hud.setTaskBusy(true)
    try {
      const result = await renameThread(thread, name)
      thread.title = result.name
      applyThreads(threads)
      hud.finishRename()
      hud.toast('Session renamed in Codex')
    } catch (error) { hud.toast(error.message, 'err') }
    finally { hud.setTaskBusy(false) }
  },
  archiveThread: async () => {
    const thread = threads.find(t => t.id === selectedId)
    if (!thread || hud.taskBusy) return
    if (thread.canArchive === false) return hud.toast('This session is managed in Dev Studio.')
    if (thread.running) return hud.toast('Wait until this task finishes before archiving it.')
    hud.setTaskBusy(true)
    try {
      ...
      select(null, {})
      applyThreads(threads)
      hud.toast('Archived in Codex · restore from Archived sessions')
      colony.ship.ping()
      await poll()
    } catch (error) { hud.toast(error.message, 'err') }
    finally { hud.setTaskBusy(false) }
  ...
  restoreThread: async id => {
    ...
    try {
      ...
      state.archived = state.archived.filter(item => item !== id)
      delete state.archivedAt[id]
      queueSave()
      applyThreads(threads)
      hud.toast('Restored in Codex · older tasks still follow your history filter')
      await poll()
    } catch (error) { hud.toast(error.message, 'err') }
    finally { hud.setTaskBusy(false) }
  },
  saveProject: (displayName, badge) => {
    if (!selectedProject) return
    state.projects ||= {}
    state.projects[selectedProject] = { displayName: displayName.trim().slice(0,48), badge }
    queueSave()
    applyThreads(threads)
    hud.toast('Headquarters sign updated')
  },
  sectionColor: id => sectionColor(id, state.sectionColors?.[id]),
  saveSectionColor: color => {
    if (!selectedId || !/^#[0-9a-f]{6}$/i.test(color)) return
    state.sectionColors ||= {}
    state.sectionColors[selectedId] = color
    queueSave()
    applyThreads(threads)
  },
...
if (!studioEmbed) chat = new CityChat(app, hud, () => void poll())
// The sidebar is permanent, so the card beside an astronaut has a wall to stay clear of.
const sideWidth = () => (studioChat ? studioInsets.right : chat && !chat.layer.hidden ? chat.quiet ? 0 : chat.$('.city-chat-panel').getBoundingClientRect().width + 32 : window.innerWidth <= 820 ? 0 : 334)
hud.setSideWidth(sideWidth())
window.addEventListener('resize', () => hud.setSideWidth(sideWidth()))
if (studioEmbed) window.addEventListener('message', event => {
  // This bridge controls presentation only, and only the frame's actual host can use it.
  if (event.source !== window.parent || event.data?.type !== 'bot-crossing:studio-ui' || typeof event.data.hideSidebar !== 'boolean') return
  studioChat = event.data.hideSidebar
  app.classList.toggle('studio-chat', studioChat)
  hud.setSideWidth(sideWidth())
  if (typeof event.data.openSettings === 'boolean') hud.toggleSettings(event.data.openSettings)
})
if (studioEmbed) window.addEventListener('message', event => {
  if (event.source !== window.parent) return
  if (event.data?.type === 'bot-crossing:studio-layout') {
    const { right, bottom } = event.data
    if (Number.isFinite(right) && Number.isFinite(bottom)) {
      studioInsets = { right: Math.max(0, Math.min(innerWidth * .65, right)), bottom: Math.max(0, Math.min(innerHeight * .6, bottom)) }
      hud._bottomInset = studioInsets.bottom
      hud.setSideWidth(sideWidth())
    }
  }
  if (event.data?.type === 'bot-crossing:studio-threads' && Array.isArray(event.data.threads)) {
    studioThreads = event.data.threads.filter(t => typeof t.id === 'string' && t.id.startsWith('studio:') && typeof t.project === 'string' && t.project.trim().length > 0)
    if (assetsReady) applyThreads(scannerThreads)
  }
  if (event.data?.type === 'bot-crossing:session-result') {
    hud.toast(event.data.error || 'Opened in Dev Studio', event.data.error ? 'err' : '')
  }
})

// ── selection ─────────────────────────────────────────────────────────────────────────

function select(id, { fly = false, react = false } = {}) {
  selectedId = id
  let agent = id ? colony.agentFor(id) : null
  if (id && !agent && colony.threads.has(id)) {
    const thread = colony.threads.get(id), building = colony.buildings.get(id)
    agent = { id, thread, status: statusFor(thread), trim: new THREE.Color(0x61a8f5), eye: new THREE.Color(1, 1, 1), pos: building.mesh.position.clone() }
  }
  if (!agent) {
    selectedId = null
  ...
  colony.astronauts.setSelected(colony.agentFor(id) || null)
  if (react) colony.astronauts.greet(id)
...
function syncProject() {
  ...
  const list = [...colony.threads.values()]
    ...
    .map((thread) => ({
      ...
      keepVisible: thread.keepVisible,
  ...
  hud.setProject({
    name: plot.name,
    identity: projectIdentity(plot.name, state.projects?.[plot.name]),
    accent: plot.accent,
...
function applyThreads(list) {
  if (studioEmbed) list = mergeStudioThreads(list, studioThreads)
  threads = list
  const view = historyView(list, { days: settings.get('historyDays'), kept: state.kept, archived: state.archived })
  const stats = colony.setThreads(view.visible, new Set(), view.projects, state.projects || {}, state.sectionColors || {})
  hud.setHistory(view)
  hud.setStats(stats)
  ...
  legendProjects = colony.plotOrder
    .map((plot) => ({
      ...
      identity: projectIdentity(plot.name, state.projects?.[plot.name]),
      accent: plot.accent,
      count: view.visible.filter(t => t.project === plot.name).length,
  ...
  if (selectedId) {
    if (colony.threads.has(selectedId)) select(selectedId, {})
    else select(null, {})
  }
...
async function poll() {
  ...
  try {
    ...
    scannerThreads = res.threads || []
    applyThreads(scannerThreads)
    hud.removeBoot()
  } catch (err) {
...
settings.onChange((changed, scope) => {
  ...
  if (changed.has('maxAgents') || changed.has('historyDays')) applyThreads(threads)
})
...
engine.add({
  update(dt, elapsed) {
    ...
    if (selectedId) {
      ...
      const agent = colony.agentFor(selectedId) || hud.selected?.agent
