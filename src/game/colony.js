...
import { projectIdentity } from './history.js'
import { sectionColor } from './sections.js'
import { createProjectSign } from '../world/project-sign.js'
...
import { ProgressBubbles } from '../agents/progress-bubbles.js'
...
export class Colony {
  constructor(scene, settings, camera, renderer) {
    ...
    this.progressBubbles = new ProgressBubbles(document.getElementById('app'))
   ...
   */
  setThreads(threads, archivedIds = new Set(), projectNames = [], identities = {}, sectionColors = {}) {
    this.projectIdentities = identities
    ...
    const byProject = new Map(projectNames.map(name => [name, []]))
    for (const thread of live) {
      if (thread.isMini) continue
    ...
    for (const [name, list] of projects) {
      ...
      list.forEach((thread, i) => {
        ...
        const building = this._syncBuilding(thread, plot, i + 1)
    ...
    let helperIndex = 0
    for (const thread of live.filter(t => t.isMini)) {
      const parentId = this.buildings.has(thread.parentId) ? thread.parentId : thread.rootParentId
      const building = this.buildings.get(parentId)
      const parent = roster.find(entry => entry.id === parentId)
      if (!building || !parent) continue
      const angle = helperIndex++ * 2.4
      const site = parent.site.clone().add(new THREE.Vector3(Math.cos(angle) * 1.2, 0, Math.sin(angle) * 1.2))
      roster.push({ id: thread.id, thread, status: 'working', sectionColor: building.accent, site, anchor: building.mesh.position.clone() })
      stats.agents++; stats.working++
      active.add(building.plot)
    }
    ...
    // Resolve sites against this scan's completed map, including enclosed courtyards.
    const arrival = this.ship.shipDoor()
    for (const entry of roster) {
      const point = this.nav.reachablePoint(arrival.x, arrival.z, entry.site.x, entry.site.z)
      if (point) entry.site.set(point.x, 0, point.z)
    }
  ...
  _syncPlots(projects) {
    ...
    const layout = allocateCells(
      projects.map(([name, list]) => ({ id: name, size: list.length + 1 })),
      this.plotCells
    )
    ...
    projects.forEach(([name], index) => {
      ...
      const hq = createBuilding({ seed: hashString(name), accent, kind: 'tower' })
      hq.scale.set(1.15, 1.65, 1.15)
      const slot = plot.worldSlot(0)
      hq.position.set(slot.x - plot.center.x, DECK_TOP, slot.z - plot.center.z)
      plot.headquarters = hq
      plot.group.add(hq)
      const podium = new THREE.Mesh(new THREE.CylinderGeometry(2.0, 2.2, 0.35, 6), new THREE.MeshStandardMaterial({ color: accent, metalness: 0.35, roughness: 0.5 }))
      podium.position.copy(hq.position).y += 0.06
      plot.group.add(podium)
    })

    this.plotOrder = [...this.plots.values()]
    for (const plot of this.plotOrder) {
      const identity = projectIdentity(plot.name, this.projectIdentities?.[plot.name])
      const sign = identity.icon + '  ' + identity.displayName + '  ·  ' + (identity.badge === 'website' ? 'WWW HQ' : 'HQ')
      if (plot.signText === sign) continue
      if (plot.label) { this.labelGroup.remove(plot.label); plot.label.userData.dispose?.() }
      plot.signText = sign
      plot.label = createProjectSign(identity, plot.accent)
      plot.label.traverse(object => { object.userData.projectId = plot.id })
      const slot = plot.worldSlot(0)
      plot.label.position.set(slot.x, DECK_TOP, slot.z)
  ...
  _syncBuilding(thread, plot, index) {
    ...
    const accent = sectionColor(thread.id, this.sectionColors?.[thread.id])
    ...
    if (!entry) {
      ...
      const pad = new THREE.Mesh(new THREE.RingGeometry(2.02, 2.2, 6).rotateX(-Math.PI / 2), new THREE.MeshStandardMaterial({ color: accent, emissive: accent, emissiveIntensity: .3, roughness: .75, polygonOffset: true, polygonOffsetFactor: -1, polygonOffsetUnits: -1 }))
      pad.position.y = .025
      mesh.add(pad)
      mesh.userData.sectionPad = pad
    ...
    entry.mesh.userData.sectionPad.material.color.set(accent)
    entry.mesh.userData.sectionPad.material.emissive.set(accent)
    entry.retiring = false
    entry.mesh.userData.sectionPad.visible = true
    return entry
  }
  ...
  _removeBuilding(id, entry) {
    ...
    entry.retiring = true
    entry.mesh.userData.sectionPad.visible = false
    entry.target = 0
    if (entry.progress <= 0.02) {
      ...
      entry.mesh.userData.sectionPad.geometry.dispose()
      entry.mesh.userData.sectionPad.material.dispose()
  ...
  _rebuildNavigation() {
    ...
    for (const plot of this.plotOrder) {
      if (plot.headquarters) {
        const p = plot.worldSlot(0)
        obstacles.push({ x: p.x, z: p.z, r: 2.25 + AGENT_RADIUS })
      }
  ...
  /** Select the physical sign only when its actual geometry is visible. */
  pickLabel(ndcX, ndcY) {
    if (!this.settings.get('showLabels')) return null
    const ray = this._signRay || (this._signRay = new THREE.Raycaster())
    ray.setFromCamera({ x: ndcX, y: ndcY }, this.camera)
    const signs = this.plotOrder.map(plot => plot.label).filter(sign => sign?.visible)
    const hit = ray.intersectObjects(signs, true)[0]
    if (!hit) return null
    const obstruction = ray.intersectObjects([this.worldGroup, this.plotGroup, this.ship.group], true)
      .find(item => item.object.visible && item.distance < hit.distance - 0.02)
    return obstruction ? null : this.plots.get(hit.object.userData.projectId) || null
  }
  ...
  /** Signs are part of the city and remain present when the HUD is hidden. */
  _updateLabels() {
    for (const plot of this.plotOrder) {
      if (plot.label) plot.label.visible = Boolean(this.settings.get('showLabels'))
    }
  }
  ...
  update(dt, elapsed, focus) {
    ...
    this.progressBubbles.update(this.astronauts.agents, this.camera)
  ...
  _badgeFor(agent) {
    if (this.progressBubbles.has(agent.id)) return BADGE.none
