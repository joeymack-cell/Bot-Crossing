...
Desktop fork note: Codex is implemented in `codex.mjs` and is the adapter enabled by the Windows app. Its chat connection lives in `../chat.mjs`; rename/archive/restore use `codex-control.mjs`. The guide below describes the general adapter seam.
...
- **Read-only by default.** Scans do not mutate session records. Chat, rename, and archive/restore are separate explicit user actions.
...
- **Codex CLI** — transcripts in `~/.codex/sessions/YYYY/MM/DD/rollout-<iso>-<uuid>.jsonl`,
  ...
  `~/.codex/session_index.jsonl`. Implemented in `codex.mjs`, including read-only SQLite metadata and JSONL fallback.
