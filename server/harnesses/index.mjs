...
export const HARNESSES = [codex, claudeCode]
...
export async function detectedHarnesses() {
  const enabled = process.env.BOT_CROSSING_HARNESSES?.split(',').map(s => s.trim()).filter(Boolean)
  const flags = await Promise.all(
    HARNESSES.map(async (h) => {
      if (enabled?.length && !enabled.includes(h.id)) return false
      try {
