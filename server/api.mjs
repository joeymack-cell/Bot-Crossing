...
import { chatView, sendChat, stopChat, answerChat, decorateChatThreads, hasChat } from './chat.mjs'
import {
  defaultHarness,
  harnessAppStartedAt,
  harnessStatus,
 ...
 * read-only during scans. Chat, rename, and archive/restore are explicit user actions.
 */
const emptyState = () => ({
...
async function readState() {
  try {
    ...
    return {
      ...
      kept: asArray(raw.kept),
      projects: asObject(raw.projects),
      sectionColors: asObject(raw.sectionColors),
...
async function writeState(next) {
  const state = {
    ...
    kept: asArray(next.kept),
    projects: asObject(next.projects),
    sectionColors: asObject(next.sectionColors),
...
async function reconcileArchived(threads) {
  ...
  return Promise.all(
    threads.map(async (thread) => {
      if (thread.harness === 'codex' || !wanted.has(thread.id)) return thread
...
function isLocalRequest(req) {
  ...
  if (origin && origin !== 'null') {
    try { const parsed = new URL(origin); return parsed.protocol === 'http:' && parsed.host === req.headers.host && LOCAL_HOSTS.has(parsed.hostname) } catch { return false }
  }
...
export async function apiMiddleware(req, res, next) {
  ...
  try {
    if (url.pathname === '/api/threads' && req.method === 'GET') {
      const threads = decorateChatThreads(await reconcileArchived(await scanThreads()))
    ...
    if (url.pathname.startsWith('/api/chat/')) {
      if (process.env.BOT_CROSSING_PROJECT_ROOT) return send(res, 403, { error: 'Use the native Dev Studio chat.' })
      if (url.pathname === '/api/chat/thread' && req.method === 'GET') {
        const id = url.searchParams.get('id')
        if (hasChat(id)) return send(res, 200, await chatView(id))
        const known = (await scanThreads()).find(thread => thread.id === id)
        if (!known) return send(res, 404, { error: 'That session is no longer available.' })
        const view = await chatView(id)
        if (known.running) { view.running = true; view.activity = 'Working in Codex · replies update here' }
        return send(res, 200, view)
      }
      if (url.pathname === '/api/chat/send' && req.method === 'POST') return send(res, 200, await sendChat(await readJsonBody(req), { resolveFolder, knownThreads: await scanThreads() }))
      if (url.pathname === '/api/chat/stop' && req.method === 'POST') return send(res, 200, await stopChat((await readJsonBody(req)).id))
      if (url.pathname === '/api/chat/answer' && req.method === 'POST') return send(res, 200, answerChat(await readJsonBody(req)))
    }
    ...
    if (url.pathname === '/api/rename' && req.method === 'POST') {
      const { harness, ref, name } = await readJsonBody(req)
      const result = await renameThread(harness, ref, name)
      return send(res, result.ok ? 200 : 400, result)
    }
    ...
    if (url.pathname === '/api/archive' && req.method === 'POST') {
      ...
      if (typeof archived !== 'boolean' || (harness === 'codex' && id !== 'codex:' + ref.threadId)) return send(res, 400, { ok: false, error: 'Invalid archive request' })
      const result = await setThreadArchived(harness, ref, archived)
      if (harness === 'codex' && !result.ok) return send(res, 400, result)
