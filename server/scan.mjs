...
import { scopeThreads } from './project-scope.mjs'
...
export async function scanThreads() {
  ...
  const threads = disambiguateProjects(scopeThreads(lists.flat()))
...
export const renameThread = (harnessId, ref, name) => {
  const h = dispatch(harnessId)
  return h.rename ? h.rename(ref, name) : { ok: false, error: 'Rename this task in its original app.' }
}
